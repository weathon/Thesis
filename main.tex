...
We then fine tune our model on the real-world dataset. With the fixed validation set, we did a hyperparamater sweep using Weight and Bias \autocite{wandb} to select models with highest validation token-level accuracy. Hyper parameter search space is shown in Table \ref{tab:hyper}.

\begin{table}[]
    \centering
    \begin{tabular}{|c|c|}
    \hline
     Hyperparameter Name  & Range  \\ \hline
      Learning Rate   &  0.0005-0.001 \\
      Learning Rate Decay & 0.95-1\\
      Epoch &  1-4\\
      Dropout between encoder and decoder &  0.0-0.5\\\hline
    \end{tabular}
...
\section{Results}
We tested how the perfomance of our model behevious with different beam size and number of images on a subset of validation set. 
\begin{figure}
    \centering
